...
class ZinicStore {
  ...
  private mapVendor(v: any): Vendor {
    return {
      ...
      name: v.name,
      // Fix: Use contactPerson instead of contact_person to match Vendor type
    ...
    try {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'requests' }, (payload) => {
          // Trigger notification for new orders received via Realtime
  ...
  private async fetchInitialData(clientId: string, isGlobal: boolean) {
    ...
    try {
      ...
      if (itemsRes.data) {
        this.items = itemsRes.data.map(i => this.mapItem(i));
        // Check for low stock on hydration
        this.items.forEach(item => {
          if (item.currentStock <= item.minStockLevel) {
            notificationService.notifyLowStock(item.name, item.currentStock);
          }
        });
      }
  ...
  async createRequest(req: StockRequest) {
    if (this.isDemoMode) {
      ...
      this.saveDemoStorage();
      notificationService.notifyNewOrder(newReq.id, newReq.department);
      this.notify();
      return;
    }
    ...
    if (data?.[0]) {
      notificationService.notifyNewOrder(data[0].id, data[0].department);
    }
  }
    ...
    if (this.isDemoMode) {
      ...
      if (item) {
        ...
        if (newStock <= item.minStockLevel) {
          notificationService.notifyLowStock(item.name, newStock);
        }
      }
      this.saveDemoStorage();
      this.notify();
  ...
  async createHotelWithAdmin(hotel: Hotel, admin: any) {
    const { error: hotelError } = await supabase.from('hotels').insert({
      ...
      // Fix: Use hotel.maxUsers, hotel.maxItems, hotel.contactEmail to match Hotel type
