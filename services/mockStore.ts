...
class ZinicStore {
  ...
  async login(clientId: string, username: string, password: string): Promise<{user: User, hotel: Hotel, isDemo?: boolean} | null> {
    ...
    try {
      ...
      // If user is SUPERADMIN, bypass mandatory hotel check
  ...
  async updateItem(item: InventoryItem) {
    ...
    const { error } = await supabase.from('inventory').upsert({
      ...
      min_stock_level: item.minStockLevel, // Fix: Use camelCase from model
  ...
  async saveVendor(vendor: Vendor) {
    ...
    const payload = {
      ...
      name: vendor.name,
      // Fix: Use camelCase property from Vendor model
