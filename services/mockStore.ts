...
class ZinicStore {
    ...
    try {
      ...
      if (localDemo) {
        ...
        if (!isExpired && demoData.clientId === normalizedClientId && username === 'admin' && demoData.password === password) {
             ...
             // Fix: Map demo admin permissions to valid UserPermission strings
             ...
             // Ensure this user is in our local users array for the demo
  ...
  async createHotelWithAdmin(hotel: Hotel, admin: any) {
    ...
    // Fix: Map new admin permissions to valid UserPermission strings
  ...
  async consumeRequestItem(reqId: string, itemId: string, quantity: number, userId: string, remark: string, userName: string) {
    ...
    await supabase.from('requests').update({ items: updatedItems, status: newStatus }).eq('id', reqId);
  ...
  async adjustStock(itemId: string, amount: number) {
    ...
    if (item) {
      await supabase.from('inventory').update({ current_stock: Math.max(0, item.current_stock + amount), last_updated: new Date().toISOString() }).eq('id', itemId);
    }
  }
