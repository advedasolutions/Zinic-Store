...
import { Hotel, UserRole, Department, DemoLead, UserPermission } from '../types';
...
import { 
  ...
  Users, Calendar, Clock, Phone, ExternalLink, LayoutGrid
} from 'lucide-react';

const PROVISION_MODULES = [
  { id: 'inventory', label: 'Stock Catalog', perms: ['inventory:view', 'inventory:add', 'inventory:modify', 'audit:view', 'audit:modify', 'transfers:view', 'transfers:modify'] },
  { id: 'requests', label: 'Requisition & Consumption', perms: ['requests:view', 'requests:add', 'requests:modify'] },
  { id: 'reports', label: 'Business Intelligence', perms: ['reports:view'] },
  { id: 'users', label: 'Team Directory', perms: ['users:view', 'users:add', 'users:modify'] },
  { id: 'vendors', label: 'Vendor Finance', perms: ['vendors:view', 'vendors:add', 'vendors:modify'] }
];
...
export const SuperAdmin: React.FC = () => {
  ...
  const [selectedModules, setSelectedModules] = useState<string[]>(PROVISION_MODULES.map(m => m.id));
  ...
  const handleOpenModal = (hotel: Hotel | null = null) => {
    ...
    setSuccessMsg(''); 
    setIsDeleting(false);
    setSelectedModules(PROVISION_MODULES.map(m => m.id));
  ...
  const handleSaveHotel = async (e: React.FormEvent) => {
    ...
    const permissions: UserPermission[] = ['settings:view', 'support:view', 'dashboard:view'];
    PROVISION_MODULES.forEach(m => {
      if (selectedModules.includes(m.id)) {
        permissions.push(...(m.perms as UserPermission[]));
      }
    });
    ...
    try {
      if (editingHotel) {
        ...
        // In a real app, we would also update the hotel_admin user's permissions here.
    ...
    <div className="space-y-10 animate-fade-in pb-20 max-w-7xl mx-auto w-full">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 lg:p-10 bg-brand-navy/60 backdrop-blur-xl animate-fade-in overflow-y-auto no-scrollbar">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl my-auto animate-reveal overflow-hidden flex flex-col">
            <div className="p-8 lg:p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Infrastructure Provisioning</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="p-3 text-slate-300 hover:text-rose-500 transition-all"><X size={32} /></button>
            </div>

            <form onSubmit={handleSaveHotel} className="p-8 lg:p-10 space-y-10 overflow-y-auto max-h-[70vh] no-scrollbar">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                {/* Left: Identity & Limits */}
                <div className="space-y-8">
                  ...
                  {!editingHotel && (
                    <div className="space-y-6 pt-6 border-t border-slate-100">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-black text-brand-navy uppercase tracking-widest">Primary Owner Profile</span>
                      </div>
                      <Input label="Owner Name" value={formData.adminFullName} onChange={e => setFormData({...formData, adminFullName: e.target.value})} required />
                      <Input label="Owner Email" type="email" value={formData.adminEmail} onChange={e => setFormData({...formData, adminEmail: e.target.value})} required />
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <Input label="Secret Key" type="password" value={formData.adminPassword} onChange={e => setFormData({...formData, adminPassword: e.target.value})} required />
                      </div>
                    </div>
                  )}
                </div>

                {/* Right: Module Provisioning */}
                <div className="space-y-8">
                  <div className="space-y-6">
                    <div className="flex items-center gap-2">
                      <LayoutGrid size={16} className="text-brand-accent" />
                      <span className="text-[11px] font-black text-brand-navy uppercase tracking-widest">Module Provisioning Matrix</span>
                    </div>
                    
                    <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 space-y-3">
                       <div className="flex items-center justify-between p-4 bg-white/50 rounded-2xl border border-emerald-100">
                          <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Base Config & Support</span>
                          <CheckCircle2 size={16} className="text-emerald-500" />
                       </div>
                       
                       {PROVISION_MODULES.map(module => (
                         <button 
                           key={module.id}
                           type="button"
                           ...
                           className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${
                             ...
                             ? 'bg-brand-navy text-brand-accent border-brand-navy shadow-lg' 
                             : 'bg-white text-slate-400 border-slate-100 hover:border-slate-300'
                           }`}
                         >
                           <span className="text-[10px] font-black uppercase tracking-widest">{module.label}</span>
                           <div className={`w-5 h-5 rounded-md flex items-center justify-center border ${selectedModules.includes(module.id) ? 'bg-brand-accent border-brand-accent text-brand-navy' : 'bg-slate-50 border-slate-200'}`}>
                              {selectedModules.includes(module.id) && <Check size={14} strokeWidth={4} />}
                           </div>
                         </button>
                       ))}
                    </div>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-4 leading-relaxed">Only selected modules will be visible to the tenant owner. Settings and Support are open by default.</p>
                  </div>
                  ...
                  {editingHotel && (
                    <div className="space-y-6 pt-6 border-t border-slate-100">
                      ...
                      <Input label="Reset Owner Key" type="password" placeholder="Leave blank to maintain" value={formData.newResetPassword} onChange={e => setFormData({...formData, newResetPassword: e.target.value})} />
                      <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[2rem]">
                        <span className="text-[11px] font-black text-brand-navy uppercase tracking-widest">Cluster Status</span>
                        <button type="button" onClick={() => setFormData({...formData, isActive: !formData.isActive})} className={`relative inline-flex h-8 w-16 items-center rounded-full transition-colors ${formData.isActive ? 'bg-emerald-500' : 'bg-slate-300'}`}>
              ...
              {formError && <div className="p-5 bg-rose-50 border border-rose-100 text-rose-600 text-[10px] font-black rounded-3xl flex items-center gap-3 animate-shake"><AlertCircle size={18}/> {formError}</div>}
              {successMsg && <div className="p-5 bg-emerald-50 border border-emerald-100 text-emerald-600 text-[10px] font-black rounded-3xl flex items-center gap-3"><CheckCircle2 size={18}/> {successMsg}</div>}

              <div className="flex gap-4 pt-6 border-t border-slate-100 shrink-0">
                {editingHotel && <Button type="button" variant="ghost" className="!text-rose-500 !bg-rose-50" onClick={handleDeleteTenant} isLoading={isDeleting}><Trash2 size={22}/></Button>}
                <Button type="button" variant="secondary" className="flex-1 !rounded-[1.5rem] h-16" onClick={() => setShowModal(false)}>Discard</Button>
                <Button type="submit" className="flex-[2] !bg-brand-navy !text-brand-accent shadow-2xl !rounded-[1.5rem] h-16 !font-black !uppercase !tracking-widest" isLoading={isSaving}><Save size={20} className="mr-2"/> Synchronize Protocol</Button>
              </div>
            </form>
          </div>
        </div>
