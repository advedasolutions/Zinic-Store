...
export const Requests: React.FC = () => {
  ...
  const [processingStatus, setProcessingStatus] = useState<RequestStatus | null>(null);
  ...
  const canApprove = useMemo(() => {
    ...
    return isAdmin || perms.includes('requests:modify');
  }, [session.user, isAdmin]);
                     
  const canConsume = useCallback((request: StockRequest) => {
    if (!session.user) return false;
    const perms = session.user.permissions || [];
    const isRequester = request.requesterId === session.user.id;
    return isAdmin || perms.includes('requests:modify') || isRequester;
  }, [session.user, isAdmin]);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-24 lg:pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-4">
          {loading && requests.length === 0 ? (
            [1,2,3].map(i => <div key={i} className="h-24 bg-slate-50 animate-pulse rounded-[2.5rem]" />)
        ...
        <div className="lg:col-span-5">
          {selectedRequest ? (
            <div className="bg-white rounded-[3rem] shadow-2xl p-8 lg:p-10 space-y-8 animate-fade-in border border-slate-100 lg:sticky lg:top-10">
                  ...
                  return (
                    <div key={idx} className="p-5 bg-slate-50 rounded-[2rem] border border-slate-100 space-y-4">
                       <div className="flex justify-between items-center">
                          ...
                          {selectedRequest.status === RequestStatus.TRANSFERRED && canConsume(selectedRequest) && (
                            <button 
              ...
              <div className="pt-8 border-t border-slate-50 space-y-4">
                {selectedRequest.status === RequestStatus.PENDING && canApprove && (
                   <div className="grid grid-cols-2 gap-4">
                      <Button 
                        className="h-16 !bg-brand-navy !text-brand-accent shadow-xl" 
                        onClick={() => handleStatusUpdate(selectedRequest.id, RequestStatus.APPROVED)}
                        isLoading={processingStatus === RequestStatus.APPROVED}
                      >
                        Authorize
                      </Button>
                      <Button 
                        ...
                        isLoading={processingStatus === RequestStatus.REJECTED}
                      >
                        Deny
                      </Button>
                   </div>
                ...
                {selectedRequest.status === RequestStatus.APPROVED && canApprove && (
                   <div className="space-y-4">
                      ...
                      <Button 
                        className="w-full h-16 !bg-emerald-600 !text-white shadow-xl" 
                        onClick={() => handleStatusUpdate(selectedRequest.id, RequestStatus.TRANSFERRED)}
                        isLoading={processingStatus === RequestStatus.TRANSFERRED}
                      >
      ...
      {showConsumeModal && consumingItem && (
        <div className="fixed inset-0 z-[150] flex items-start md:items-center justify-center p-4 bg-brand-navy/80 backdrop-blur-xl animate-fade-in overflow-y-auto no-scrollbar pt-10 md:pt-4">
           <div className="bg-white rounded-[3.5rem] shadow-2xl w-full max-w-md overflow-hidden p-10 animate-reveal my-auto">
