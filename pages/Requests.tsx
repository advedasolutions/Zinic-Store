...
export const Requests: React.FC = () => {
  ...
  const canApprove = session.user?.role === UserRole.SUPERADMIN || 
                     session.user?.role === UserRole.HOTEL_ADMIN || 
                     session.user?.permissions?.includes('APPROVE_REQUESTS');
                     
  const canConsume = session.user?.role === UserRole.SUPERADMIN || 
                     session.user?.role === UserRole.HOTEL_ADMIN || 
                     session.user?.permissions?.includes('MANAGE_CONSUMPTION');
